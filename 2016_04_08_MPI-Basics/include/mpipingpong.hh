...
void nonblocking_comm(const int32_t& rank, const int32_t& rank_i, const int32_t& rank_j, const uint32_t& mcount, float* sendbuffer, float* recbuffer) {
  ...
  if (rank == rank_i) {
    MPI_Request req;
    MPI_Isend(sendbuffer, mcount, MPI_FLOAT, rank_j, 0, MPI_COMM_WORLD, &req); // rank 0 post send to rank 1
    MPI_Wait(&req,MPI_STATUS_IGNORE);
    MPI_Irecv(recbuffer, mcount, MPI_FLOAT, rank_j, 0, MPI_COMM_WORLD, &req); // rank 0 post recieve from rank 1
    MPI_Wait(&req,MPI_STATUS_IGNORE);
  }
  if (rank == rank_j) {
    MPI_Request req;
    MPI_Irecv(recbuffer, mcount, MPI_FLOAT, rank_i, 0, MPI_COMM_WORLD, &req); // rank 1 post recieve from rank 0
    MPI_Wait(&req,MPI_STATUS_IGNORE);
    MPI_Isend(sendbuffer, mcount, MPI_FLOAT, rank_i, 0, MPI_COMM_WORLD, &req); // rank 1 post send to rank 0
    MPI_Wait(&req,MPI_STATUS_IGNORE);
  }
}
  ...
  auto benchmark = [&](
                  ...
                  float*, float*)> callable)
    {
      ...
      if ((rank==rank_i && recbuffer[mcount-1]==rank_j) || (rank==rank_j && recbuffer[mcount-1]==rank_i)) {
        if (rank==rank_i) cout << "Rank "<<rank<<" - Minimum time: " << tperformance*1e3 << " ms\n";
      }
  };

  // benchmarks
  if (rank == rank_i) cout << "    Blocking ";
  benchmark(blocking_comm); start = 0, end = 0, elapsed = 0, tperformance = 1e8;
  if (rank == rank_i) cout << "Non-blocking ";
  benchmark(nonblocking_comm); start = 0, end = 0, elapsed = 0, tperformance = 1e8;
  if (rank == rank_i) cout << "Head-to-head ";
  benchmark(headtohead_comm);
