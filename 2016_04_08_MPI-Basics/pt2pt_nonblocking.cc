...
int main(int argc, char** argv) {
  ...
  for (int i=0; i<ntrials; i++)
  {
    ...
    start = MPI_Wtime();
    if (rank==0) {
      MPI_Isend(sendbuffer, mcount, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, &req[1]); // rank 0 post send to rank 1
      MPI_Irecv(recbuffer, mcount, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, &req[2]); // rank 0 post recieve from rank 1
      MPI_Wait(&req[1],&stat[1]);
      MPI_Wait(&req[2],&stat[2]);
    }
    if (rank ==1) {
      MPI_Irecv(recbuffer, mcount, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &req[0]); // rank 1 post recieve from rank 0
      MPI_Isend(sendbuffer, mcount, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &req[3]); // rank 1 post send to rank 0
